...
| Upstream status | 🔴 **Not fixed upstream** — verified on `upstream/main` at v8.0.1 (2026-09-19): no reference to `client_status` or `key_storage_status` anywhere in `apps/backend/src`, and `@owf/token-status-list` 0.3.2 and 0.4.0 both read only the top-level `status`. No issue or PR mentions it. Issue to be filed |
